...
**ChainCommand** 是一個多代理 AI 系統，能夠自主優化供應鏈運營。十個專業代理——分佈在戰略層、戰術層和作業層——通過事件驅動架構協作，實現需求預測、庫存管理、風險評估、物流協調和執行報告生成。

系統在 Mock 模式下可完全自主運行（無需 API 金鑰），自動生成合成數據、訓練 ML 模型，並執行完整的決策週期。代理之間透過 pub/sub 事件通訊，高成本行動自動升級至人工審批，並持續優化 KPI 指標。
